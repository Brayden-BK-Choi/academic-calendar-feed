...
# 이벤트 설명(DESCRIPTION)에 포함할 속성들, DB 스키마의 실제 프로퍼티 이름과 맞춰야 함
DESCRIPTION_PROPERTIES = ["비고", "시간, 장소", "담당자"]
# "아동명부(출결)"/"아동명부(상담)" 관계형 속성에서 학생 이름을 롤업(rollup)해오는 속성들.
# 관계형(relation) 속성 자체는 Notion API에서 페이지 ID만 반환되어 이름을 알 수 없으므로,
# 이름 텍스트로 이미 변환되어 있는 롤업 속성을 사용한다.
STUDENT_ROLLUP_PROPERTIES = {"출결": "출결 대상 학생", "상담": "상담 대상 학생"}
TITLE_PROPERTY = "일정명"
DATE_PROPERTY = "날짜"
DEPT_PROPERTY = "부서"
LINK_PROPERTY = "링크"
    ...
    if t == "rollup":
        ...
        if rtype == "array":
            ...
            for item in rollup.get("array", []):
                it = item.get("type")
                if it == "title":
                    parts.append("".join(x["plain_text"] for x in item.get("title", [])))
                elif it == "rich_text":
                    parts.append("".join(x["plain_text"] for x in item.get("rich_text", [])))
            return ", ".join(p for p in parts if p)
        if rtype == "number":
            return str(rollup.get("number")) if rollup.get("number") is not None else ""
        return ""
    return ""
...
def build_event(page):
    ...
    for prop_name, display_label in STUDENT_ROLLUP_PROPERTIES.items():
        val = get_text(props.get(prop_name))
        if val:
            desc_parts.append(f"{display_label}: {val}")
